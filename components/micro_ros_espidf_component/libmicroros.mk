...
$(EXTENSIONS_DIR)/micro_ros_src/src:
    ...
    test -e src/rcl_logging/rcl_logging_log4cxx && touch src/rcl_logging/rcl_logging_log4cxx/COLCON_IGNORE || :; \
    test -e src/rcl_logging/rcl_logging_spdlog && touch src/rcl_logging/rcl_logging_spdlog/COLCON_IGNORE || :; \
    test -e src/rclc/rclc_examples && touch src/rclc/rclc_examples/COLCON_IGNORE || :; \
	test -e src/rcl/rcl_yaml_param_parser && touch src/rcl/rcl_yaml_param_parser/COLCON_IGNORE || :; \
...
# ESP32-S3, ESP32-P4, ESP32: need workaround in IDF 5.4+
ifeq ($(IDF_TARGET),$(filter $(IDF_TARGET),esp32s3 esp32p4 esp32))
	@if [ $(IDF_VERSION_MAJOR) -gt 5 ] || ([ $(IDF_VERSION_MAJOR) -eq 5 ] && [ $(IDF_VERSION_MINOR) -ge 4 ]); then \
		echo $(UROS_DIR)/atomic_workaround; \
		mkdir $(UROS_DIR)/atomic_workaround; cd $(UROS_DIR)/atomic_workaround; \
		$(X_AR) x $(UROS_DIR)/install/lib/librcutils.a; \
		$(X_STRIP) atomic_64bits.c.obj --strip-symbol=__atomic_fetch_add_8; \
